    ...
    ROWS.forEach((row) => {
      if (row.section) {
        ...
        const checksHtml = row.checks?.map((check) =>
          check
            ? `<td class="text-center"><div class="check">&#10003;</div></td>`
            : `<td></td>`
        ).join('') || '';
        rowsHtml += `<tr><td class="col-name">${row.name}</td><td class="col-date">${row.date}</td>${checksHtml}</tr>`;
      }
    });
    const headersHtml = SHORT_HEADERS.map(h => `<th class="col-header">${h}</th>`).join('');

    const htmlContent = `<!DOCTYPE html>
<html lang="fr"><head><meta charset="UTF-8"><title>Tableau de synthèse - JUNG Jean-Marie</title>
<style>
...
<body>
  ...
  <div class="header-info">
    <div class="info-block"><span class="info-label">Candidat</span><span class="info-value">JUNG Jean-Marie</span></div>
    <div class="info-block"><span class="info-label">Centre / Option</span><span class="info-value">EPSI <span class="pill">SISR</span></span></div>
    <div class="info-block"><span class="info-label">Portfolio</span><span class="info-value" style="font-family:monospace;color:#2563eb;font-size:12px;">https://www.netcy.fr/portfolio</span></div>
  </div>
  <table>
    <thead>
      <tr>
  ...
  return (
    <section id="tableau" className="relative scroll-mt-32">
      <div className="mb-12">
        <h2 className="text-3xl font-bold text-on-surface mb-4">Tableau de synthèse BTS SIO</h2>
      ...
      <div className="flex justify-center md:hidden">
        <button
          ...
          className="flex items-center gap-2 px-5 py-3 bg-surface-container border border-outline-variant rounded-lg text-sm font-medium text-on-surface hover:bg-surface-container-high transition-colors"
        >
      ...
      <div className="hidden md:block bg-surface-container-lowest border border-outline-variant rounded-xl overflow-visible shadow-sm -mx-4 lg:-mx-8">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between p-4 bg-surface-container-low border-b border-outline-variant gap-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 flex-1">
            <div className="flex flex-col justify-center">
              ...
              <span className="text-on-surface font-semibold text-sm">JUNG Jean-Marie</span>
            </div>
            <div className="flex flex-col justify-center">
              <span className="text-outline uppercase tracking-widest font-bold mb-1 text-[10px]">Centre / Option</span>
              <div className="flex items-center gap-2">
                <span className="text-on-surface font-semibold text-sm">EPSI</span>
                <span className="px-1.5 py-0.5 rounded text-[10px] bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 font-bold">SISR</span>
              </div>
            </div>
            <div className="flex flex-col justify-center">
              <span className="text-outline uppercase tracking-widest font-bold mb-1 text-[10px]">Portfolio</span>
              <a href="https://www.netcy.fr/portfolio" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 font-mono text-xs hover:underline">
          ...
          <button
            onClick={handleDownloadPDF}
            className="flex items-center gap-2 px-4 py-2 bg-surface-container hover:bg-surface-container-high border border-outline-variant rounded-lg text-sm font-medium text-on-surface transition-colors whitespace-nowrap"
          >
            <Download className="w-4 h-4" />
            Télécharger le PDF
          </button>
        ...
        <table className="w-full text-left border-collapse table-fixed">
          ...
          <thead className="bg-surface-container">
            <tr>
              <th className="p-4 border-r border-outline-variant align-bottom">
                <span className="text-xs font-bold text-on-surface-variant uppercase tracking-widest block">Réalisations & Périodes</span>
              </th>
              <th className="p-3 border-r border-outline-variant align-bottom text-center">
                <span className="text-[10px] text-outline uppercase font-mono tracking-tighter">Période</span>
              </th>
              {SHORT_HEADERS.map((header, i) => (
                <th key={i} className="group relative p-3 border-r border-outline-variant align-bottom text-center">
                  <div className="text-[11px] font-bold text-on-surface leading-tight cursor-help border-b border-dashed border-outline inline-block pb-1">
                    {header}
                  </div>

                  {/* Tooltip */}
                  <div className={`absolute z-50 bottom-full mb-3 w-[260px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 pointer-events-none ${i >= 4 ? 'right-0' : 'left-1/2 -translate-x-1/2'}`}>
                    <div className="bg-surface-container-highest border border-outline-variant rounded-lg p-4 shadow-xl text-left">
                      <h4 className="text-xs font-bold text-blue-600 dark:text-blue-400 mb-3 border-b border-outline-variant pb-2">{FULL_DETAILS[i].title}</h4>
                      <ul className="space-y-1.5">
                        {FULL_DETAILS[i].items.map((item, j) => (
                          <li key={j} className="text-[10px] text-on-surface-variant leading-snug break-words">{item}</li>
                        ))}
                      </ul>
                    </div>
                    <div className={`absolute top-full -mt-px border-4 border-transparent border-t-surface-container-highest ${i >= 4 ? 'right-8' : 'left-1/2 -translate-x-1/2'}`} />
                  </div>
                </th>
              ))}
            </tr>
